...
function patchCompositorDragLifecycle(source) {
  ...
  if (needsKWinStart) {
    ...
    patched = replaceMethodText(
      patched,
      startMethod,
      `${startMethod.text.slice(0, -1)},this.codexPetOverlayBeginKWinDrag(${windowMatch[1]})}`,
    );
  }
  ...
  if (endMethod.text.includes("codexPetOverlayEndKWinDrag(")) {
    return patched;
  }
  ...
  return replaceMethodText(
    ...
    endMethod.text.slice(0, completionMatch.index) +
      `this.codexPetOverlayEndKWinDrag(${windowVar},()=>{${completionNeedle}})||(()=>{${completionNeedle}})()` +
      "}",
  );
}
